...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/DaanVanVugt/ruby-mc/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("mc_forecast.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in mc_forecast.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "mc_forecast.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
